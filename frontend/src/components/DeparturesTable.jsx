...
const DeparturesTable = ({ selectedDate, setSelectedDate, flightData = [], loading = false, onRefresh }) => {
  ...
  const filteredDeparturesCount = useMemo(() => {
    return departures.filter(row => row.operational_status?.toLowerCase() !== 'not operating').length;
  }, [departures]);

  const totalFiltered = departures.filter(
  ...
  useEffect(() => {
    intervalRef.current = setInterval(() => {
      if (autoCycling) {
        setCurrentPage((prev) => {
          const totalPagesCalc = Math.ceil(filteredDeparturesCount / PAGE_SIZE);
          return totalPagesCalc === 0 ? 1 : (prev % totalPagesCalc) + 1;
        });
      }
    }, 5000);
    return () => clearInterval(intervalRef.current);
  }, [filteredDeparturesCount, autoCycling, timerResetKey]);
  ...
  const renderStatus = (row) => {
    ...
    const stod = new Date(row.stod);
    const etod = new Date(row.etod);
    const now = currentTime;

    if (now.getTime() >= etod.getTime()) {
      return <span className="flight-status-departed">Departed</span>;
    }

    const boardingThreshold = new Date(now.getTime() + (40 * 60 * 1000));
    if (etod.getTime() <= boardingThreshold.getTime()) {
      return <span className="flight-status-boarding">Boarding</span>;
    }
    ...
    <div className="overflow-visible">
      ...
      >
        <div className="flex gap-3 mb-5 items-center relative" style={{ zIndex: 9999 }}>
          <label htmlFor="departures-date-input" className="text-sm font-medium text-slate-700">Select Date:</label>
                    ...
                    >
                      ...
                      <td className="table-cell table-date-cell">
                        <div className="table-date-time">
                          <div className="font-medium">{formatDate(row.stod)}</div>
                          <div className="text-slate-500 text-sm">{formatTime(row.stod)}</div>
                        </div>
                      </td>
                      ...
                      <td className="table-cell table-date-cell">
                        {row.etod === '-' ? (
                          ...
                          <div className="table-date-time">
                            <div className="font-medium">{formatDate(row.etod)}</div>
                            <div className="text-slate-500 text-sm">{formatTime(row.etod)}</div>
                          </div>
                        )}
                      </td>
                      <td className="table-cell">{row.dep_boarding_gate_no}</td>
                      <td className="table-cell">{row.pstd}</td>
